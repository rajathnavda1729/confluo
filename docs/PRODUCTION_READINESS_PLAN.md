...
| ID | Item | Source § | Description | Acceptance criteria |
|----|------|----------|-------------|---------------------|
| **P1.1** | Structured logging | 3.1 Operability | Introduce a small logger interface (e.g. `Logger` with `Info`, `Warn`, `Error`) and optional JSON output. Use it in consumer, delay, timeout, and main. Support log level (e.g. env `LOG_LEVEL=info`). Optionally add a request/correlation ID when processing a record (e.g. partition+offset or trace ID). | Logs are parseable (e.g. JSON); level configurable; key events (handle error, commit error, egress failure) logged with level and context. |
| **P1.2** | Capacity and partition guidance | 3.3 Scaling | Add an "Operations" or "Capacity" section to ARCHITECTURE.md or new docs/OPERATIONS.md: recommend partition count vs target throughput and consumer count; mention key cardinality impact on Scylla/Redis; Bloom filter capacity and error rate tuning. | Ops doc exists with partition, Scylla, Redis, and Bloom tuning guidance. |
| **P1.3** | Commit failure retry | 3.5 Resilience | In consumer `Run`, on `CommitRecords` failure, retry a bounded number of times (e.g. 3) with backoff before returning/exiting. Log each retry. | Transient commit failures do not exit immediately; after N failures, process still exits for orchestration to restart. |
| **P1.4** | Bloom operational note | 3.2 Durability | Document in OPERATIONS or ARCHITECTURE: Bloom false positives cause an extra Scylla read; safe but adds load. Optionally add a metric for "first arrival" vs "follow-up" (e.g. counter by path) to tune Bloom. | Documented; optional metric if implemented. |
